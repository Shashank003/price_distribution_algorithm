...
originalPricePool = pricePoolBeforeRunningCharges - ((25/100)*pricePoolBeforeRunningCharges)
totalPrizePoolRemaining = originalPricePool

print("Price after cut pool: "+ str(originalPricePool))
...
for i, rank in enumerate(npRankArray):
    ...
    if (rank >3 and rank < 11):
        yourReward = (0.5/100)*originalPricePool
        totalPrizePoolRemaining = totalPrizePoolRemaining - yourReward
        print(f"person: {i}, rank: {rank}")
        print("Your reward is:   " + str(yourReward))
    ...
    if (rank >=11 and rank < 26):
        yourReward = (0.35/100)*originalPricePool
        totalPrizePoolRemaining = totalPrizePoolRemaining - yourReward
        print(f"person: {i}, rank: {rank}")
        print("Your reward is:   " + str(yourReward))
